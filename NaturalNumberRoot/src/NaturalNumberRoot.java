...
public final class NaturalNumberRoot {
    ...
    public static void root(NaturalNumber n, int r) {
        ...
        // interval [low, high) where the answer must be
        // low starts at 0, high starts at n+1
        ...
        // compute high - low to check interval
        NaturalNumber diff = high.newInstance();
        diff.copyFrom(high);
        diff.subtract(low);

        while (diff.compareTo(one) > 0) {
            // pick the midpoint of the current interval
            NaturalNumber mid = low.newInstance();
            mid.copyFrom(low);
            mid.add(high);
            // mid = (low + high) / 2
            mid.divide(two);

            // compute mid^r to compare against n
            NaturalNumber midPow = mid.newInstance();
            midPow.copyFrom(mid);
            midPow.power(r);
            ...
            // update diff for the next iteration
            diff.copyFrom(high);
            diff.subtract(low);
        }

        //low is the root rounded down
        n.copyFrom(low);
    }
